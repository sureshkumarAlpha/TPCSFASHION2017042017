...
public class VariantGridMaker extends GridMaker{

	 
	public String formVariantGrid(Map<String, String> visColMap,List<String> listSelectedColumns, List<Map<String,String>> data, int ipageno, ResourceBundle bundle,UserRights VariantRights,PageConfig pc) throws IOException{
	    ...
	    try{  
	      ...
	      for(int i=0,j=1,size=data.size();i<size;i++,j++){ 
	        ...
	        if(!equal)
	        {
	        strBuffer.append(getActionCellForVariants(map,ipageno,bundle,objRights));
	        }else{
	        	  strBuffer.append("&nbsp;");
	        }
        strBuffer.append("</td>");
...
//	    	strBuffer.append("</div>");
	        ...
	        boolean equal = previous_order_no.equals(map.get("Variant.VariantId"));
	        if(!equal)
	        {
	        	strBuffer.append("<td width=\"10%\">");
	        strBuffer.append(getActionCellForVariants(map,ipageno,bundle,VariantRights));
	        }else{
	        	strBuffer.append("<td >");
	        	  strBuffer.append("&nbsp;");
	        }
        ...
        strBuffer.append(getToggleCheckCellForVariant(map,ipageno,bundle,VariantRights));
        }else{
        	  strBuffer.append("<td>&nbsp;</td>");
        }
  	        strBuffer.append("</tr>"); 
  	        previous_order_no=map.get("Variant.VariantId"); 
	...
	private String getToggleCheckCellForVariant(Map<String, String> map, int ipageno,ResourceBundle bundle, UserRights VariantRights) {
		StringBuffer buffer=new StringBuffer();
		buffer.append("<td><div class=\"checkbox\">  <input class=\"checkbox_1\" type=\"checkbox\" id=\"mat_check_"+map.get("Variant.VariantId")+"\" name=\"bulkvariantId\" value="+map.get("Variant.VariantId")+" /><label for=\"mat_check_"+map.get("Variant.VariantId")+"\" class=\"checkbox_1\"  ></label></div></td>");
	...
	private String getActionCellForVariants(Map<String, String> Variant, int ipageno,ResourceBundle bundle, UserRights VariantRights) {
		 int permitted=1;
		 StringBuffer buffer=new StringBuffer();
			
		 buffer.append("<select style=\" align:left;width: 97px; \"  class=\"form-control\" id=\"select_action\" name=\"select_action\" onchange=\"displayVariantActionList(this.value,'"+ipageno+"','"+Variant.get("Variant.VariantId")+"')\">");
		 buffer.append("<option selected=\"selected\" value=\"-1\">"+bundle.getString("Common.SelectAction")+"</option>");
//		 System.out.println("VARIANT ID===="+Variant.get("Variant.VariantId"));
			 if(VariantRights.getDeletePermission()==permitted){
		 buffer.append("<option value=\"1\">"+bundle.getString("Button.Delete")+"</option>");
			 }
			 if(VariantRights.getEditPermission()==permitted){
...
/*public class MaterialGridMaker extends GridMaker{
  ...
  public String formMaterialGrid(Map<String, String> visColMap,List<String> listSelectedColumns, List<Map<String,String>> data,UserRights objRights) throws IOException{
	...
	private String getActionCellForVariants(Map<String, String> map, int ipageno,ResourceBundle bundle, UserRights rights) {
		 int permitted=1;
		 StringBuffer buffer=new StringBuffer();
