...
public class ApprovedPriceListGridMaker {

	
	public String formApprovedPriceListGrid(TPCSUser userInfo,Map<String, String> visColMap,List<String> listSelectedColumns, List<Map<String,String>> data, int ipageno, ResourceBundle bundle,UserRights seRights,UserRights sqRights,PageConfig pc) throws IOException{
		...
		try{  
			...
			for(int i=0,size=data.size();i<size;i++){ 
				...
				strBuffer.append("<tr class=\"datarow\" id=\"enq_row_"+map.get("ApprovedPriceList.SupplierId")+"\"  > <td  >" );
//				strBuffer.append("<tr class=\"datarow\"> <td  >" );
				boolean equal = previous_order_no.equals(map.get("ApprovedPriceList.SupplierId"));
				if(!equal)
				{
...
//				strBuffer.append("<td align=\"left\" valign=\"middle\" onclick=\"showEnquiryDetailgrid("+map.get("ApprovedPriceList.PurchasePriceId")+");\">"+map.get("SalesEnquiry.EnquiryNo")+"</td>");
				...
				while(it2.hasNext()){
					...
					strBuffer.append("<td align=\"left\" valign=\"middle\" onclick=\"showEnquiryDetailgrid("+map.get("ApprovedPriceList.SupplierId")+");\">");
				...
				strBuffer.append("</tr>"); 
				previous_order_no=map.get("ApprovedPriceList.SupplierId"); 
	...
	private String getActionCellForSalesEnquiry(Map<String, String> map, int ipageno,ResourceBundle bundle, UserRights seRights,UserRights sqRights) {
		...
		buffer.append("<select style=\" align:left;width: 97px; \" id=\"select_action\" class=\"form-control\" name=\"select_action\" onchange=\"displayApprovedPriceListActionList(this.value,'"+ipageno+"','"+map.get("ApprovedPriceList.SupplierId")+"','"+map.get("ApprovedPriceList.PurchasePriceId")+"')\">");
	...
	private String getToggleCheckCellForSalesEnquiry(Map<String, String> map, int ipageno,ResourceBundle bundle, UserRights seRights,UserRights sqRights) {
		StringBuffer buffer=new StringBuffer();
		buffer.append("<td><div class=\"checkbox\"  > <input class=\"checkbox_1\" type=\"checkbox\" id=\"se_check_"+map.get("ApprovedPriceList.SupplierId")+"\" name=\"SupplierApl_Id\" value="+map.get("ApprovedPriceList.SupplierId")+"   /><label for=\"se_check_"+map.get("ApprovedPriceList.SupplierId")+"\" class=\"checkbox_1\"  ></label></div></td>");
