...
$(window).load(function(){
            // Bind normal buttons
			Ladda.bind( 'div:not(.progress-demo) .ladda-button', { timeout: 1500 } );
...
$(window).load(function(){
            // Bind normal buttons
			Ladda.bind( 'div:not(.progress-demo) .btn-group .ladda-button', { timeout: 1500 } );

			// Bind progress buttons and simulate loading progress
			Ladda.bind( '.progress-demo .btn-group button', {
				callback: function( instance ) {
					var progress = 0;
					var interval = setInterval( function() {
						progress = Math.min( progress + Math.random() * 0.1, 1 );
